-- ============================================================================
-- 0010 — Cotización de envío en vivo (Skydropx) en el checkout
--
-- shipping_quotes guarda cada cotización que la Edge Function obtiene de
-- Skydropx (escrita con service_role; sin políticas RLS = invisible e
-- inmodificable para el cliente). El navegador elige tarifa mandando
-- (quote_id, rate_id) y create_order relee el precio de aquí: elegir tarifa
-- nunca es elegir precio.
-- ============================================================================
...
create index shipping_quotes_created_idx on public.shipping_quotes(created_at);

alter table public.shipping_quotes enable row level security;
-- Sin políticas: nadie lee ni escribe desde el cliente. Solo service_role
-- (la función de cotización) y create_order (security definer).

-- ----------------------------------------------------------------------------
-- create_order aprende a cobrar la tarifa real elegida.
-- ----------------------------------------------------------------------------
drop function if exists public.create_order(jsonb, text, text, text, jsonb, text, text);
  ...
  if p_quote_id is not null and p_rate_id is not null then
    ...
    if found then
      ...
      if v_rate_cost is not null then
        ...
        -- necesita saber con qué paquetería y a qué costo real mandar.
