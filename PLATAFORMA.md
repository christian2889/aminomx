...
DiDi (o con un asociado propio): la tienda solo ofrece la opción. En un CP
de Ensenada la entrega es **exclusivamente local**: `skydropx-rates` ni
siquiera cotiza paquetería y el checkout muestra solo "Entrega local ·
$100" (gratis desde el umbral nacional de $1,900, que aplica `create_order`
en el servidor). Al pagar, el pedido pasa a `processing`, la línea de tiempo
instruye al cliente a coordinar por WhatsApp y el panel muestra los pasos
(sin guía Skydropx; repartidor/rastreo se capturan a mano). Config en
`settings`, key `local_delivery`: `{ enabled, provider, service, cost_cents,
min_subtotal_cents, cp_prefixes }` — `cost_cents` es el precio y
`min_subtotal_cents` el "gratis desde" que se muestra al cliente.
